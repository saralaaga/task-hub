...
describe("Task Hub styles", () => {
  ...
  it("keeps the calendar creation modal narrow with aligned controls", () => {
    ...
    const noteComposerDatePickerRule = styles.match(/\.task-hub-note-composer-date-picker\s*\{(?<body>[^}]+)\}/)?.groups?.body ?? "";
    ...
    expect(noteComposerDatePickerRule).toContain("border-bottom: 1px solid var(--background-modifier-border)");
    expect(noteComposerDatePickerRule).toContain("display: grid");
    expect(noteComposerDatePickerInputRule).toContain("appearance: none");
    expect(noteComposerDatePickerInputRule).toContain("font-variant-numeric: tabular-nums");
    expect(noteComposerDatePickerInputRule).toContain("width: 100%");
  ...
  it("keeps dated note list cards readable and detail Markdown compact", () => {
    ...
    const scrollerRule = styles.match(/\.task-hub-dated-note-detail::(?:-webkit-scrollbar),\s*\.task-hub-dated-note-list::(?:-webkit-scrollbar)\s*\{(?<body>[^}]+)\}/)?.groups?.body ?? "";
    const scrollerTrackRule = styles.match(/\.task-hub-dated-note-detail::(?:-webkit-scrollbar-track),\s*\.task-hub-dated-note-list::(?:-webkit-scrollbar-track)\s*\{(?<body>[^}]+)\}/)?.groups?.body ?? "";
    const scrollerThumbRule = styles.match(/\.task-hub-dated-note-detail::(?:-webkit-scrollbar-thumb),\s*\.task-hub-dated-note-list::(?:-webkit-scrollbar-thumb)\s*\{(?<body>[^}]+)\}/)?.groups?.body ?? "";
    ...
    const detailBodyRule = styles.match(/^\.task-hub-dated-note-body\s*\{(?<body>[^}]+)\}/m)?.groups?.body ?? "";
    ...
    const activeCardRule = styles.match(/button\.task-hub-dated-note-card\.is-active\s*\{(?<body>[^}]+)\}/)?.groups?.body ?? "";
    const excerptRule = styles.match(/^\.task-hub-dated-note-excerpt\s*\{(?<body>[^}]+)\}/m)?.groups?.body ?? "";
    const activeExcerptRule = styles.match(/button\.task-hub-dated-note-card\.is-active\s+\.task-hub-dated-note-excerpt\s*\{(?<body>[^}]+)\}/)?.groups?.body ?? "";
    ...
    expect(detailRule).toContain("scrollbar-color:");
    expect(scrollerRule).toContain("background: transparent");
    expect(scrollerTrackRule).toContain("background: transparent");
    expect(scrollerTrackRule).toContain("border: 0");
    expect(scrollerTrackRule).toContain("box-shadow: none");
    expect(scrollerThumbRule).toContain("border: 0");
    expect(transitionRule).toContain("animation: task-hub-dated-note-detail-enter 140ms");
    expect(detailCardRule).toContain("border: 1px solid var(--background-modifier-border)");
    expect(detailCardRule).toContain("border-radius: 8px");
    expect(detailCardRule).toContain("box-shadow:");
    expect(detailCardRule).not.toContain("border-right: 0");
    ...
    expect(detailBodyRule).not.toContain("border: 1px solid var(--background-modifier-border)");
    ...
    expect(activeCardRule).toContain("background: transparent");
