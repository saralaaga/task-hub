...
const JA_SETTINGS_TRANSLATIONS: Partial<Record<TranslationKey, string>> = {
  ...
  datedNotesFolder: "ノートビューの保存フォルダー",
  datedNotesFolderDesc:
    "ノートビューから作成した新規ノートはここへ保存されます。Thino に表示したい場合は、このフォルダーを Thino の multi-file 保存先に合わせて、下の Thino メタデータ設定も有効にしてください。",
  ...
  eventCreationDefaultTargetDesc: "カレンダーから新規イベントを作成するときに既定で使う作成先。",
  ...
  taskNotesAddThinoId: "ノートビューのノートに Thino ID メタデータを書き込む",
  taskNotesAddThinoIdDesc:
    "ノートビューで新規作成したノートへ id / createdAt / updatedAt を書き込みます。これだけで Thino への表示が保証されるわけではなく、Thino が監視しているフォルダー内にある必要があります。",
...
const FR_SETTINGS_TRANSLATIONS: Partial<Record<TranslationKey, string>> = {
  ...
  datedNotesFolder: "Dossier d'enregistrement des notes",
  datedNotesFolderDesc:
    "Les nouvelles notes créées depuis la vue Notes sont enregistrées ici. Pour que Thino puisse les afficher, pointez ce dossier vers le dossier multi-fichier de Thino et activez aussi le réglage de métadonnées Thino ci-dessous.",
  ...
  eventCreationDefaultTargetDesc: "Destination utilisée par défaut lors de la création d'un événement depuis le calendrier.",
  ...
  taskNotesAddThinoId: "Écrire les métadonnées d'identifiant Thino dans les notes de la vue Notes",
  taskNotesAddThinoIdDesc:
    "Écrit id / createdAt / updatedAt dans les nouvelles notes de la vue Notes. Cela ne garantit pas à lui seul l'affichage dans Thino ; la note doit aussi se trouver dans un dossier indexé par Thino.",
...
const KO_SETTINGS_TRANSLATIONS: Partial<Record<TranslationKey, string>> = {
  ...
  datedNotesFolder: "노트 보기 저장 폴더",
  datedNotesFolderDesc:
    "노트 보기에서 만든 새 노트가 여기에 저장됩니다. Thino에 보이게 하려면 이 폴더를 Thino multi-file 폴더로 맞추고 아래 Thino 메타데이터 설정도 켜세요.",
  ...
  eventCreationDefaultTargetDesc: "캘린더에서 새 이벤트를 만들 때 기본으로 사용할 생성 위치입니다.",
  ...
  taskNotesAddThinoId: "노트 보기 노트에 Thino ID 메타데이터 쓰기",
  taskNotesAddThinoIdDesc:
    "노트 보기에서 새로 만든 노트에 id / createdAt / updatedAt을 기록합니다. 이것만으로 Thino 표시가 보장되지는 않으며, 노트가 Thino가 색인하는 폴더 안에도 있어야 합니다.",
...
const TRANSLATIONS: Record<"en" | "zh", Record<TranslationKey, string>> &
  ...
  en: {
    ...
    datedNotesDisabled: "Turn on Notes view in Task Hub settings first.",
    datedNotesEnable: "Notes view",
    datedNotesEnableDesc: "Show the Notes view in Task Hub and allow creating standalone Markdown notes from Task Hub.",
    datedNotesFolder: "Notes view save folder",
    datedNotesFolderDesc:
      "New notes created from the Notes view are saved here. To let Thino pick them up, point this folder at your Thino multi-file folder and enable the Thino metadata toggle below.",
    ...
    datedNoteRelatedTaskMissing: "Linked task unavailable",
    day: "Day",
    ...
    eventCreationDefaultTargetDesc: "Default destination used when you create an event from the calendar.",
    ...
    taskNotesDefaultModeDesc: "Create related Task Hub notes or notes compatible only with Thino multi-file storage.",
    ...
    taskNotesAddThinoId: "Write Thino ID metadata to Notes view notes",
    taskNotesAddThinoIdDesc:
      "Write id, createdAt, and updatedAt to new Notes view notes. This alone does not guarantee Thino will show the note; the note still needs to live in a folder Thino indexes.",
    taskNotesThino: "Enable Thino compatibility for task notes",
    taskNotesThinoDesc:
      "Show Thino multi-file compatible options for task and event notes. Thino single-file, Canvas, and diary storage are not generated.",
  ...
  zh: {
    ...
    datedNotesDisabled: "请先在 Task Hub 设置中开启笔记视图。",
    datedNotesEnable: "笔记视图",
    datedNotesEnableDesc: "启用 Task Hub 的笔记视图，并允许从 Task Hub 创建普通 Markdown 笔记。",
    datedNotesFolder: "笔记视图保存目录",
    datedNotesFolderDesc: "从笔记视图新建的笔记会保存到这里。想让 Thino 检索到它们，请把这里设为 Thino 的 multi-file 目录，并开启下方的 Thino 元数据开关。",
    datedNotesOpenAfterCreate: "创建后打开笔记",
    datedNotesOpenAfterCreateDesc: "创建 Markdown 笔记后在新标签页打开。",
    datedNoteRelatedTaskMissing: "关联任务已失效",
    day: "日",
    defaultView: "默认视图",
    ...
    eventCreationDefaultTargetDesc: "从日历新建事件时默认创建到的位置。",
    ...
    taskNotesDefaultModeDesc: "创建与任务或日历事件关联的 Task Hub 笔记，或仅兼容 Thino multi-file 存储的笔记。",
    ...
    taskNotesAddThinoId: "给笔记视图中的笔记写入 Thino ID 元数据",
    taskNotesAddThinoIdDesc:
      "给笔记视图中新建的笔记写入 id、createdAt、updatedAt。仅写入这些字段并不保证 Thino 一定显示这篇笔记；它还需要位于 Thino 当前会检索的文件夹中。",
    taskNotesThino: "为任务笔记启用 Thino 兼容选项",
    taskNotesThinoDesc: "为任务和日历事件关联笔记显示 Thino multi-file 兼容选项；不生成 Thino single-file、Canvas 或日记存储内容。",
  ...
  ja: {
    ...
    settingsPageAdvanced: "詳細",
    settingsPageCalendar: "カレンダー",
    settingsPageIntegrations: "連携",
    settingsPageOverview: "概要",
    settingsPageTasks: "タスク",
    saturday: "土曜日",
    sendTo: "送信先",
  ...
  fr: {
    ...
    settingsPageAdvanced: "Avancé",
    settingsPageCalendar: "Calendrier",
    settingsPageIntegrations: "Intégrations",
    settingsPageOverview: "Vue d'ensemble",
    settingsPageTasks: "Tâches",
    saturday: "Samedi",
    sendTo: "Envoyer vers",
  ...
  ko: {
    ...
    settingsPageAdvanced: "고급",
    settingsPageCalendar: "캘린더",
    settingsPageIntegrations: "연동",
    settingsPageOverview: "개요",
    settingsPageTasks: "작업",
    saturday: "토요일",
    sendTo: "보내기",
