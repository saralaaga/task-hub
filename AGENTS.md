...
- `docs/superpowers/specs/2026-07-11-hub-note-protocol.zh-CN.md`：统一笔记 frontmatter、`taskhub-kind`、统一 ID 生成规则和兼容策略。后续如果修改关联笔记、日期笔记、外部导入笔记、语音转写笔记或任何 Hub Note 的 YAML 协议，先读这份文档，再动实现。
...
- 如果改动涉及关联笔记、日期笔记、外部导入笔记、语音转写笔记或任何 Hub Note 的 YAML frontmatter，必须先阅读 `docs/superpowers/specs/2026-07-11-hub-note-protocol.zh-CN.md`，并优先复用 `src/hubNotes.ts` 的统一 parser / writer / ID 生成入口；不要在局部功能里手写新的笔记协议。
